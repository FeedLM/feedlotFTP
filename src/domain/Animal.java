...
public class Animal extends ExportTable {
    ...
    public void cargarDatos_1(ManejadorBD bd, Date fecha) {
        ...
        bd.consulta(""
                + "SELECT a.id_animal,                      COALESCE(id_proveedor,''),                  \n"
                + "       COALESCE(fecha_compra, '1900-01-01 00:00:00'),                     COALESCE(compra,''),                        \n"
                + "       COALESCE(numero_lote,''),                      COALESCE(peso_compra,0.0),     \n"
                + "       COALESCE(id_sexo,''),                          COALESCE(fecha_ingreso, '1900-01-01 00:00:00'),                 \n"
                + "       arete_visual,                     COALESCE(arete_electronico,''),             \n"
                + "       COALESCE(arete_siniiga,''),                     COALESCE(arete_campaña,''),                 \n"
                + "       COALESCE(peso_actual,0.0),        COALESCE(temperatura,0),                   \n"
                + "       COALESCE(es_semental,'N'),        COALESCE(id_semental,0),       \n"
                + "       COALESCE(id_raza, ''),            COALESCE(es_vientre,'N'),                    \n"
                + "       COALESCE(fecha_recepcion, '1900-01-01 00:00:00'),                  COALESCE(peso_recepcion,0.00), \n"
                ...
                + "       COALESCE(fecha_ultima_comida, '1900-01-01 00:00:00'),              COALESCE(ganancia_promedio,0.00), \n"
                + "       a.status \n"
                + "FROM   animal a, repl_animal r \n"
                + "WHERE  a.id_animal =   r.id_animal \n"
